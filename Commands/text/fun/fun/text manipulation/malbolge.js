...
*/
        ...
        const toseed = [mb.opc.nop, mb.opc.rot, mb.opc.opr];
        const toseed_norot = [mb.opc.nop, mb.opc.opr];

        function parseTargetString(target) {
            let t, escape = false, ret = [];
...
*/
        function generateStaticText(target, callback, randomPool) {
            ...
            let gmvm = generateBoilerplatte(randomPool),
                mvm = gmvm;
            let g, window = [],
                t, tt, norot = false;
        ...
        function buildPrefix(mcode, canJump, bestWithoutJump, randomPool) {
            ...
            if (!canJump) {
                const hiscode = buildPrefix(mcode, true, lnow ? lnow : Infinity, randomPool);
